...
describe("the integration plan", () => {
  // src/platform/integration-plan.ts is regenerated by the Factory's sdk step in every game,
  // so this checks whatever plan the repository holds is one the gameplay layer can act on,
  // rather than asserting the template's empty default.
  it("the shipped plan is well-formed", () => {
    const ids = INTEGRATION_PLAN.placements.map((placement) => placement.id);
    expect(new Set(ids).size).toBe(ids.length);
    for (const placement of INTEGRATION_PLAN.placements) {
      expect(placement.id).toMatch(/\S/);
      expect(["interstitial", "rewarded", "banner"]).toContain(placement.kind);
      expect(["game-over", "level-complete", "pause-menu"]).toContain(placement.moment);
      if (placement.platforms !== null) expect(placement.platforms.length).toBeGreaterThan(0);
    }
    for (const [target, adapter] of Object.entries(INTEGRATION_PLAN.adapterSubstitutes)) {
      expect(target).not.toBe(adapter);
    }
  });

  it("a plan that places nothing shows no offer and takes no break", async () => {
    const game = new Game();
    const platform = new GenericWebPlatform({ namespace: "test" });
    const empty: IntegrationPlan = {
      titleId: "empty",
      placements: [],
      adapterSubstitutes: {},
      breakOnContinue: [],
    };
    const subject = new PlatformGameplay(game, platform, empty, { target: "poki" });
