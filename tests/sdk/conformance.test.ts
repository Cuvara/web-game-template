...
describe.each(HARNESSES)("$id", (harness) => {
  ...
  feature("game-binding", () => {
    ...
    scenario("a hidden tab pauses the game and stops reported gameplay", async () => {
      ...
      bindPlatform(game, instance.platform);
      instance.platform.gameplayStart();
      // Where the portal detects focus loss itself and asks not to be told (CrazyGames:
      // "Don't call this event when the user switches focus or leaves the game area (we
      // handle this on our side)" — https://docs.crazygames.com/sdk/game/#gameplay-startstop),
      // the game still pauses but reported gameplay is left running.
      const reports = instance.platform.capabilities.gameplayStopOnHidden ?? true;
      visibility = "hidden";
      document.dispatchEvent(new Event("visibilitychange"));
      expect(game.paused).toBe(true);
      expect(instance.platform.gameplayActive).toBe(!reports);
      visibility = "visible";
      document.dispatchEvent(new Event("visibilitychange"));
      expect(game.paused).toBe(false);
      expect(instance.platform.gameplayActive).toBe(true);
