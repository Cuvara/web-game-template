...
The live SDK script (`api.gamemonetize.com/sdk.js`, obfuscated) was inspected only to predict
behaviour the documentation leaves open — never to add API. It raises `SDK_GAME_START` after
an ad error ("Advertisement error, no worries, start / resume the game"), can raise
`SDK_GAME_PAUSE` without a request (a preroll), and raises other, undocumented events, which
the adapter ignores.
...
| Snippet already in `index.html`   | refused (no SDK): its `SDK_OPTIONS` would route events elsewhere. Do not add the documented snippet by hand — the adapter owns it                 |
...
| ad error                               | before start: `reason: "error"`; after start: the ad still closes on `SDK_GAME_START`                                                 |
| `showBanner()` throws                  | `reason: "error"`                                                                                                                     |
| callback never arrives                 | resolves `not-ready` after `adStartTimeoutMs` (10 s) — the SDK rejects premature calls silently                                       |
...
| callback arrives late                  | a pause after the request gave up is treated as the portal holding the screen: game paused and muted until `SDK_GAME_START` or 60 s   |
...
| repeated requests                      | one at a time: a concurrent second request is `busy`, never queued; sequential ones each run. No local interval (the SDK rate-limits) |
...
2. Build with the ID (`game_id` or `WGF_GAMEMONETIZE_GAME_ID`) and `ad_kinds: [interstitial]`;
   `pnpm sdk:prepare` must exit 0.
3. `pnpm release:package --release r<n>` → `release/r<n>/gamemonetize.zip`, `index.html` at the
   root, no source maps — the upload format GameMonetize documents.
4. Upload it: Game Management > My games > the game > drop the zip.
5. **Verify Game** in the dashboard — GameMonetize's check that the SDK is integrated.
6. **REQUEST ACTIVATION** — reviewed by GameMonetize's content manager.
7. Self-hosting a GameMonetize-monetized game is by arrangement only (info@gamemonetize.com).
...
| -------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| `tests/unit/gamemonetize.test.ts`                                    | 53 tests: config, boot, every ad case above, rewarded refusal, pause/resume with a real `Game` and `bindPlatform`    |
...
| `tests/sdk-matrix/gamemonetize.spec.ts` (Chromium desktop + Pixel 7) | 22 per device, through the real script loader: all adversarial cases, PixiJS and Three.js                            |
...
- **`autoplay: false`** is taken from GameMonetize's own Construct 2 plugin, not the HTML5
  README. If the SDK ignores it and plays a preroll at load, the adapter handles it as the
  portal holding the screen (paused, muted, bounded by 60 s).
...
- **Undocumented behaviour.** GameMonetize documents no error codes, no fill signal and no
  rate-limit signal; the timeouts (5 s init, 10 s ad start, 60 s ad) are this adapter's, sized
  from the observed SDK.
